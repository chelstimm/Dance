...
//   });
  ...
  var orm = {
    ...
    selectAll: function(callback) {
      ...
      // Run MySQL Query
      connection.query('SELECT * FROM dances', function (err, result) {
        if (err) throw err;
        callback(result);
      });
    ...
    // insertOne()
    insertOne: function(dance, callback){
    ...
    updateOne: function(id, callback){
      ...
      // Run MySQL Query
      connection.query('UPDATE dances SET ? WHERE ?', [{queued: true}, {id: id}], function (err, result) {
          if (err) throw err;
          callback(result);
        });
